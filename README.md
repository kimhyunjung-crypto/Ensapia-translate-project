...
현재 번역 화면은 한국어·일본어 입력, 3,000자 검사, 방향 자동 감지, 진행·결과·복사·재시도 흐름을 제공합니다. 서버는 암호화된 활성 인명·용어·말투 규칙을 찾아 인명·호칭 → 회사 용어 → 상황별 말투 순으로 고정하고, 변경하지 않은 원문과 같은 규칙 목록을 OpenAI·Gemini용 입력으로 각각 준비합니다. 실제 AI 번역 연결 전에는 화면에 명시된 연습용 데모 결과를 반환하며, OpenAI·Gemini 호출 파이프라인은 EPIC 5에서 연결합니다.
